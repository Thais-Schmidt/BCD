...
('Paulo Sergio','768.633.600-78','13502-293','192',null),
...
insert into `db_food_service`.`categoria`(`descricao`) values
('bebidas'),
('lanches'),
('porções'),
('sobremesas'),
...
insert into `db_food_service`.`produto`(`categoria_id`, `descricao`, `tipo`) values
...
INSERT INTO pedido
(cliente_id, entregador_id, data_hora_ini)
VALUES
...
# ATENÇÃO!!! 
## REALIZAR OS INSERTS DA TABELA **ITEN_PEDIDO** SOMENTE APÓS A RESOLUÇÃO DOS EXERCÍCIOS **3** E **4**!
...
### 2 - CRIAR UMA STORED PROCEDURE QUE ATUALIZE O CAMPO **VALOR_TOTAL** DA TABELA PEDIDO, AO ATUALIZAR ESSE CAMPO O NOVO VALOR DEVE 
##SER SOMADO COM O VALOR JÁ EXISTENTE NO MESMO. NOMEIE A **STORED PROCEDURE** COMO **PRC_ATUALIZA_VALOR_PEDIDO**, LEMBRE-SE QUE A MESMA DEVE 
##RECEBER PARÂMETROS NECESSÁRIOS PARA REALIZAR A OPERAÇÃO, OS PARÂMETROS DEVEM SER PASSADOS AO CHAMAR ESSA PROCEDURE NA TRIGGER QUE SERÁ CRIADA NO EXERCÍCIO 3.
 
delimiter $$
create procedure prc_atualiza_valor_pedido(in valor_item decimal(10,2), in quantidade int, in pedido_id int)
begin
    update pedido set valor_total = valor_total + (valor_item * quantidade) where id = pedido_id;
end $$
delimiter;
...
DELIMITER &&
CREATE TRIGGER TRG_ACIONA_PRC_ATUALIZA_VALOR_PEDIDO AFTER INSERT ON item_pedido FOR EACH ROW
BEGIN    
    CALL PRC_ATUALIZA_VALOR_PEDIDO(NEW.valor_item, NEW.quantidade, NEW.pedido_id);
END &&
DELIMITER ;
...
update pedido set data_hora_fim = date_add(data_hora_fim, interval 1 hour);

### 5 - SELECIONAR OS 20 PEDIDOS DE MAIOR VALOR ORDENADOS POR ORDEM DECRESCENTE;
SELECT * FROM pedidos ORDER BY valor_total DESC LIMIT 20;

### 6 - SELECIONAR OS 20 PEDIDOS DE MENOR VALOR ORDENADOS POR ORDEM CRESCENTE;
SELECT * FROM pedidos ORDER BY valor_total ASC LIMIT 20;
...
JOIN 
    entregador ON pedido.entregador_id = entregador.id;
