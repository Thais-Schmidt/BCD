...
delimiter $$
create procedure prc_atualiza_valor_pedido (
    pedido_id int,
    novo_valor decimal(10, 2)
)
begin
update pedido
    set valor_total = valor_atual + novo_valor
    where id = pedido_id;
end $$
delimiter ; 
...
create trigger trg_aciona_prc_atualiza_valor_pedido
	...
	for each row
begin
    call prc_atualiza_valor_pedido(new.pedido_id, new.valor_item);
end $$
delimiter ;
...
select id, data_hora_ini, valor_total from pedido;
